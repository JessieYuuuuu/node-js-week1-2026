...
function sumCredits(members) {
  // TODO: 實作此函式
  // 提示：用 reduce，初始值給 0
  return members.reduce((acc, item) => acc += item.credits, 0)
}
...
async function getVIPSummary(filePath) {
  ...
  const mList = await readMembers('./fixtures/members.json');
  console.log(mList)
  const isVip = filterVIP(mList)
  const count = isVip.length;
  const totalCredits = sumCredits(isVip);
  const names = isVip.map(m => m.name);
  return {
    count, totalCredits, names
  }
}
